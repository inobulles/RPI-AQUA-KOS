...
static void kos_font_create_text(kos_font_t* this, char* text) {
	if          (this->text == NULL || strcmp(text, this->text) != 0) {
		...
		sprintf(command, "convert -background transparent -antialias -fill white -font \"%s\" -pointsize %d label:'testing123' " TEMP_TEXT_RESULT_IMAGE_ROOT, "root/fonts/main.ttf", /*(int) this->size*/ 72);
		system (command);
